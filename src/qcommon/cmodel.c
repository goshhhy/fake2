...
cmodel_t *CM_LoadMap( char *name, qboolean clientload, unsigned *checksum ) {
    ...
    *checksum = last_checksum;
    ...
    if ( header.version != BSPVERSION )
        Com_Error( ERR_DROP,
                   "CMod_LoadBrushModel: %s has wrong version number (%i "
                   "should be %i)",
...
int CM_NumClusters( void ) { return numclusters; }

int CM_NumInlineModels( void ) { return numcmodels; }

char *CM_EntityString( void ) { return map_entitystring; }
...
void CM_InitBoxHull( void ) {
    ...
    if ( numnodes + 6 > MAX_MAP_NODES || numbrushes + 1 > MAX_MAP_BRUSHES ||
...
================
*/
void CM_TestBoxInBrush( vec3_t mins, vec3_t maxs, vec3_t p1, trace_t *trace,
                        cbrush_t *brush ) {
    int i, j;
    cplane_t *plane;
    float dist;
...
trace_t CM_BoxTrace( vec3_t start, vec3_t end, vec3_t mins, vec3_t maxs,
    ...
    if ( start[0] == end[0] && start[1] == end[1] && start[2] == end[2] ) {
        ...
        numleafs =
            CM_BoxLeafnums_headnode( c1, c2, leafs, 1024, headnode, &topnode );
...
byte *CM_ClusterPVS( int cluster ) {
    if ( cluster == -1 )
        ...
        CM_DecompressVis( map_visibility + map_vis->bitofs[cluster][DVIS_PVS],
                          pvsrow );
    return pvsrow;
}
...
byte *CM_ClusterPHS( int cluster ) {
    if ( cluster == -1 )
        ...
        CM_DecompressVis( map_visibility + map_vis->bitofs[cluster][DVIS_PHS],
                          phsrow );
    return phsrow;
}
