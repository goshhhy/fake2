...
void PM_CheckJump( void ) {
    if ( pm->s.pm_flags & PMF_TIME_LAND ) {  // hasn't been long enough since
                                             // landing to jump again
        return;
    }
...
void PM_CheckDuck( void ) {
    ...
    if ( pm->s.pm_type == PM_DEAD ) {
        pm->s.pm_flags |= PMF_DUCKED;
                ...
                ( pm->s.pm_flags & PMF_ON_GROUND ) ) {  // duck
        pm->s.pm_flags |= PMF_DUCKED;
...
void Pmove( pmove_t *pmove ) {
    ...
    if ( pm->s.pm_flags &
         PMF_TIME_TELEPORT ) {  // teleport pause stays exactly in place
    } else if ( pm->s.pm_flags &
                PMF_TIME_WATERJUMP ) {  // waterjump has no control, but falls
        pml.velocity[2] -= pm->s.gravity * pml.frametime;
        if ( pml.velocity[2] <
             0 ) {  // cancel as soon as we are falling down again
            pm->s.pm_flags &=
                ~( PMF_TIME_WATERJUMP | PMF_TIME_LAND | PMF_TIME_TELEPORT );
            pm->s.pm_time = 0;
        }
