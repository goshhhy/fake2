...
export fn VID_Init() void {
    ...
    if (re.api_version != c.API_VERSION) {
        c.Com_Error(c.ERR_FATAL, @ptrCast([*:0]const u8, &"Re has incompatible api_version"));
    }

    // call the init function
    if (re.Init) |reInit| {
        if (reInit (null, null) == false) {
            c.Com_Error (c.ERR_FATAL, @ptrCast([*:0]const u8, &"Couldn't start refresh"));
        }
    } else {
        c.Com_Error (c.ERR_FATAL, @ptrCast([*:0]const u8, &"Re has no init function"));
    }
}
