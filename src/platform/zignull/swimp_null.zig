...
const video = @import("vid_null.zig");
...
export fn SWimp_EndFrame() void {
    ...
    while ( y < @intCast( usize, vid.height ) ) {
        while ( x < @intCast( usize, vid.width ) ) { 
            const colorIndex = vid.buffer[ ( y * @intCast( usize, vid.width ) ) + x ]; 
            const color = currentPalette[colorIndex];
            if ( c.SDL_FillRect(pixel, 0, c.SDL_MapRGB(pixel.*.format, color.r, color.g, color.b) ) != 0 ) {
                 std.debug.warn("fill failed");               
            }

            var dest = c.SDL_Rect{ .x = @intCast( c_int, x ), .y = @intCast( c_int, y ), .w = 1, .h = 1 };
            if ( c.SDL_BlitSurface( pixel, 0, rsurface, &dest ) != 0 ) {
