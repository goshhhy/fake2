...
void R_Alias_clip_z( finalvert_t *pfv0, finalvert_t *pfv1, finalvert_t *out ) {
    ...
    scale =
        ( ALIAS_Z_CLIP_PLANE - pfv0->xyz[2] ) / ( pfv1->xyz[2] - pfv0->xyz[2] );
...
void R_Alias_clip_left( finalvert_t *pfv0, finalvert_t *pfv1,
    ...
    if ( pfv0->v >= pfv1->v ) {
        scale =
            (float)( r_refdef.aliasvrect.x - pfv0->u ) / ( pfv1->u - pfv0->u );
        ...
        scale =
            (float)( r_refdef.aliasvrect.x - pfv1->u ) / ( pfv0->u - pfv1->u );
...
void R_Alias_clip_top( finalvert_t *pfv0, finalvert_t *pfv1,
    ...
    if ( pfv0->v >= pfv1->v ) {
        scale =
            (float)( r_refdef.aliasvrect.y - pfv0->v ) / ( pfv1->v - pfv0->v );
        out->u = pfv0->u + ( pfv1->u - pfv0->u ) * scale + 0.5;
        out->v = pfv0->v + ( pfv1->v - pfv0->v ) * scale + 0.5;
        out->s = pfv0->s + ( pfv1->s - pfv0->s ) * scale + 0.5;
        out->t = pfv0->t + ( pfv1->t - pfv0->t ) * scale + 0.5;
        ...
        scale =
            (float)( r_refdef.aliasvrect.y - pfv1->v ) / ( pfv0->v - pfv1->v );
        out->u = pfv1->u + ( pfv0->u - pfv1->u ) * scale + 0.5;
        out->v = pfv1->v + ( pfv0->v - pfv1->v ) * scale + 0.5;
        out->s = pfv1->s + ( pfv0->s - pfv1->s ) * scale + 0.5;
        out->t = pfv1->t + ( pfv0->t - pfv1->t ) * scale + 0.5;
...
================
*/
void R_AliasClipTriangleRGB( finalvert_t *index0, finalvert_t *index1,
                             finalvert_t *index2 ) {
    int i, k, pingpong;
    unsigned clipflags;
