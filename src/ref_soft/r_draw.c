...
void Draw_8to24( byte *palette ) {
    ...
    // qb: gray is the new black.. d_8to24tabble[0] &= 0x000000;	// black is
    // black
}
...
byte BestColor( int r, int g, int b, int start, int stop ) {
    ...
    for ( i = start; i <= stop; i++ ) {
        ...
        db = b - (int)pal[2];
        pal += 4;
        distortion = dr * dr + dg * dg + db * db + dr * 5 + dg * 5 +
                     db * 5;  // qb: this will increase color sensitity at low
                              // brightness.  Added + dr + dg + db
...
void Draw_InitRGBMap( void ) {
...

    {
        ...
        for ( r = 0; r < 256; r += 4 ) {
            for ( g = 0; g < 256; g += 4 ) {
                for ( b = 0; b < 256; b += 4 ) {
                    // 3dfx gamma hack, trying to match the saturation and gamma
                    // of the refgl+3dfxgl combo so many q2 players are familiar
                    // with
                    ...
                    ga = ia + ( ga - ia ) * mysat;
                    ba = ia + ( ba - ia ) * mysat;
                    // beastcolor = BestColor (pow(ra / mydiv, mypow) * mydiv,
                    // pow(ga / mydiv, mypow) * mydiv, pow(ba / mydiv, mypow) *
                    // mydiv, 1, 254);
...
void Draw_StretchPicImplementation( int x, int y, int w, int h, image_t *pic ) {
    ...
    w = (int)( w / 4 ) *
        4;  // qb: for DIB, sigh... probably should be 'ifdef DIB'
