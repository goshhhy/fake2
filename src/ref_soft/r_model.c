...
void Mod_LoadSubmodels( lump_t *l ) {
    ...
    for ( i = 0; i < count; i++, in++, out++ ) {
        ...
        if ( lastface <
             out->firstface )  // || lastface > loadmodel->nummodelsurfaces)
...
void Mod_LoadFaces( lump_t *l ) {
    ...
    for ( surfnum = 0; surfnum < count; surfnum++, in++, out++ ) {
        ...
        if ( out->numedges < 3 ||
             lastedge < out->firstedge )  // || lastedge > loadmodel->numedges)
...
void Mod_LoadNodes( lump_t *l ) {
    ...
    for ( i = 0; i < count; i++, in++, out++ ) {
        ...
        if ( out->firstsurface + out->numsurfaces >
             loadmodel->numsurfaces )  // Knightmare added
...
void Mod_LoadLeafs( lump_t *l ) {
    ...
    for ( i = 0; i < count; i++, in++, out++ ) {
        ...
        if ( loadmodel->vis != NULL ) {
            if ( out->cluster < -1 ||
                 out->cluster >= loadmodel->vis->numclusters )
        ...
        out->firstmarksurface =
            ...
            (unsigned short)LittleShort(
                in->firstleafface );  // Knightmare- make sure this doesn't turn
                                      // negative!
        out->nummarksurfaces = LittleShort( in->numleaffaces );
        // Knightmare added
        lastmarksurface =
            LittleShort( in->firstleafface ) + out->nummarksurfaces;
...
void Mod_LoadBrushModel( model_t *mod, void *buffer ) {
    ...
    dmodel_t *bm;
    coloredlights =
        r_coloredlights->value;  // leilei - colored lights - sanity check
...
struct model_s *R_RegisterModel( char *name ) {
    ...
    if ( mod ) {
        ...
        if ( mod->type == mod_sprite ) {
            ...
            for ( i = 0; i < sprout->numframes; i++ )
                mod->skins[i] =
                    R_FindImage( sprout->frames[i].name, it_sprite );
