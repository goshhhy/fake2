    ...
    for ( i = 0; i < node->numsurfaces; i++, surf++ ) {
        ...
        if ( lightmap ) {
            ...
            for ( maps = 0; maps < MAXLIGHTMAPS && surf->styles[maps] != 255;
                ...
                for ( i = 0; i < 3; i++ )
                    scale[i] =
                        1 * r_newrefdef.lightstyles[surf->styles[maps]].rgb[i];
...
unsigned blocklights[1024 * 3];  // allow some very large lightmaps // leilei -
                                 // *3 added
...
void R_BuildLightMapRGB( void ) {
    ...
    // clear to ambient
    for ( i = 0; i < size; i++ ) blocklights[i] = r_refdef.ambientlight << 8;
    ...
    for ( i = 0; i < size; i++ ) {
        ...
        blocklights[i] =
            ( r < 256 )
                ? 256
                : ( r > 65536 ) ? 65536 : r;  // leilei - made min 256 to rid
                                              // visual artifacts and gain speed
    }
}
