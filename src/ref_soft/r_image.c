...
image_t *R_FindImage( char *name, imagetype_t type ) {
    ...
    len = strlen( name );
    if ( len < 5 )
        return NULL;  // ri.Sys_Error (ERR_DROP, "R_FindImage: bad name: %s",
                      // name);
    ...
    if ( !strcmp( name + len - 4, ".pcx" ) ) {
        ...
        if ( !pic )
            return NULL;  // ri.Sys_Error (ERR_DROP, "R_FindImage: can't load
                          // %s", name);
        image = GL_LoadPic( name, pic, width, height, type );
    } else if ( !strcmp( name + len - 4, ".wal" ) ) {
        image = R_LoadWal( name );
    } else if ( !strcmp( name + len - 4, ".tga" ) )
        return NULL;  // ri.Sys_Error (ERR_DROP, "R_FindImage: can't load %s in
                      // software renderer", name);
    else
        return NULL;  // ri.Sys_Error (ERR_DROP, "R_FindImage: bad extension on:
                      // %s", name);
...
===============
*/
void R_InitImages( void ) { registration_sequence = 1; }
