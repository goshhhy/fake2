...
void D_DrawTurbulent8Span( espan_t *pspan ) {
    ...
    do {
        if ( sw_transmooth->value /*> 1*/ ) {
            ...
            DitherKernel2( sturb, tturb, pspan->u + r_turb_spancount,
                           pspan->v );
            ...
            sturb = ( ( r_turb_s +
                        r_turb_turb[( r_turb_t >> 16 ) & ( CYCLE - 1 )] ) >>
                      16 ) &
                    63;
            tturb = ( ( r_turb_t +
...
void Turbulent8( espan_t *pspan ) {
    ...
    do {
        r_turb_pdest =
            (unsigned char *)( (byte *)d_viewbuffer +
...
// qb: this one does a simple motion blur, but leaves artificacts (smears on
// walls) due to alphamap imperfections. #define WRITEPDEST_MB(i)  { pdest[i] =
//vid.alphamap[*(pbase + (s >> 16) + (t >> 16) * cachewidth)*256+pdest[i]];
//s+=sstep; t+=tstep;}

// qbism: pointer to pbase and macroize idea from mankrip
#define WRITEPDEST( i )                                                 \
    {                                                                   \
        pdest[i] = *( pbase + ( s >> 16 ) + ( t >> 16 ) * cachewidth ); \
        s += sstep;                                                     \
        t += tstep;                                                     \
    }

void D_DrawSpans16( espan_t *pspan )  // qb: up it from 8 to 16.  This + unroll
                                      // = big speed gain!
{
    ...
    do {
        pdest = (byte *)( (byte *)d_viewbuffer + ( r_screenwidth * pspan->v ) +
                          pspan->u );
        count = pspan->count >> 4;
