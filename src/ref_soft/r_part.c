...
__declspec( naked ) void BlendParticle33( void ) {
    ...
    __asm mov ebp, vid.alphamap __asm xor ebx,
        ebx

        __asm mov bl,
        byte ptr[edi] __asm shl ebx,
        8
...
__declspec( naked ) void R_DrawParticle( void ) {
    ...
    */
    __asm mov ebpsave,
        ebp __asm push esi __asm push edi
    ...
    p_o.x - r_o.x | p_o.y - r_o.y |
        p_o.z - r_o.z __asm fstp dword ptr[local + 0];
    ...
    ( empty )
        ...
        */
        ...
        __asm mov eax,
        dword ptr[transformed + 8] __asm and eax,
        eax __asm js end __asm cmp eax,
        particle_z_clip __asm jl end
        ...
        */
        ...
        PARTICLE_66 __asm je blendfunc_66 __asm jl blendfunc_33 __asm lea ebx,
        BlendParticle100 __asm jmp done_selecting_blend_func blendfunc_33
        : __asm lea ebx,
          BlendParticle33 __asm jmp done_selecting_blend_func blendfunc_66
        : __asm lea ebx,
          BlendParticle66 done_selecting_blend_func
        : __asm mov blendfunc,
          ebx
    ...
    ycenter | zi *transformed[1] |
        xcenter + zi *transformed[0] __asm fsubrp st( 1 ),
        st( 0 );
    ycenter - zi *transformed[1] |
        xcenter + zi *transformed[0] __asm fxch st( 1 );
    xcenter + zi *transformed[0] |
        ycenter + zi *transformed[1] __asm fadd point_five;
    xcenter + zi *transformed[0] + 0.5 |
        ycenter - zi *transformed[1] __asm fxch st( 1 );
    ycenter - zi *transformed[1] |
        xcenter + zi *transformed[0] + 0.5 __asm fadd point_five;
    ycenter - zi *transformed[1] + 0.5 |
        xcenter + zi *transformed[0] + 0.5 __asm fxch st( 1 );
    ...
    ( empty )
        ...
        */
        ...
        __asm mov ebx,
        u __asm mov ecx, v __asm cmp ecx,
        d_vrectbottom_particle __asm jg end __asm cmp ecx,
        d_vrecty __asm jl end __asm cmp ebx,
        d_vrectright_particle __asm jg end __asm cmp ebx,
        d_vrectx __asm jl end
    ...
    esi = (short *)( d_pzbuffer + ( d_zwidth * v ) + u )
        ...
        __asm lea edi,
    [d_scantable + ecx * 4] __asm mov edi, [edi] __asm add edi,
    d_viewbuffer __asm add edi,
    ebx
    ...
    ax
        ...
        */
        ...
        __asm xor
        edx,
    edx __asm mov dx, izi __asm mov ecx, d_pix_shift __asm shr dx,
    cl

        //	if (pix < d_pix_min)
        //		pix = d_pix_min;
        __asm cmp edx,
    d_pix_min __asm jge check_pix_max __asm mov edx,
    d_pix_min __asm jmp skip_pix_clamp
