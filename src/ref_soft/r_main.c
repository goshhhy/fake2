...
cvar_t *sw_waterwarp;
cvar_t *sw_transmooth;  // texture dither //qb: was sw_texturesmooth, but just
                        // transparencies
...
void R_SetLightLevel( void ) {
    ...
    if ( ( r_newrefdef.rdflags & RDF_NOWORLDMODEL ) ||
         ( !r_drawentities->value ) || ( !currententity ) ) {
...
void R_BeginFrame( float camera_separation ) {
    ...
    */
    ...
    while ( sw_mode->modified || vid_fullscreen->modified ) {
        ...
        /*
        ** if this returns rserr_invalid_fullscreen then it set the mode but not
        *as a
        ** fullscreen mode, e.g. 320x200 on a system that doesn't support that
        *res
        */
        if ( ( err = SWimp_SetMode( &vid.width, &vid.height,
                                    sw_mode->value ) ) == rserr_ok ) {
