...
#define MIP8RGBX( i )                                            \
    {                                                            \
        ...
        if ( pix == 255 )                                        \
            ...
            pix24 = (unsigned char *)&d_8to24table[pix];         \
            trans[0] = ( pix24[0] * ( light[0] ) ) >> 17;        \
            trans[1] = ( pix24[1] * ( light[1] ) ) >> 17;        \
            trans[2] = ( pix24[2] * ( light[2] ) ) >> 17;        \
            if ( trans[0] & ~63 )                                \
                trans[0] = 63;                                   \
            if ( trans[1] & ~63 )                                \
                trans[1] = 63;                                   \
            if ( trans[2] & ~63 )                                \
                trans[2] = 63;                                   \
            prowdest[i] = palmap2[trans[0]][trans[1]][trans[2]]; \
        }                                                        \
    }
...
static void ( *surfmiptable8RGB[4] )( void ) = {
    R_DrawSurfaceBlock8RGBX_mip0, R_DrawSurfaceBlock8RGBX_mip1,
    R_DrawSurfaceBlock8RGBX_mip2, R_DrawSurfaceBlock8RGBX_mip3};

// extern	int			host_fullbrights;   // for preserving fullbrights in color
// operations extern byte		*lmmap;
extern byte palmap2[64][64][64];  //  Colored Lighting Lookup Table

// leilei - Colored Lights - End

void R_BuildLightMap( void );
// extern	unsigned		blocklights[1024];	// allow some very large
// lightmaps extern	unsigned		blocklights[1024*3];	// leilei - colored
// lights
...
void R_DrawSurface( void ) {
    ...
    if ( coloredlights )
        pblockdrawer =
            surfmiptable8RGB[r_drawsurf.surfmip];  // leilei - colored lights
    else
    ...
    basetptr = &r_source[(
        ( ( ( basetoffset >> r_drawsurf.surfmip ) + ( tmax << 16 ) ) % tmax ) *
        twidth )];
...
surfcache_t *D_SCAlloc( int width, int size ) {
    ...
    size = ( size + 3 ) & ~3;
    if ( size > sc_size )
        ri.Sys_Error( ERR_FATAL, "D_SCAlloc: %i > cache size of %i", size,
                      sc_size );
