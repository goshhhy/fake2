...
void D_ViewChanged( void ) {
    ...
    d_pix_max = (int)( (float)r_refdef.vrect.width / ( 800.0 / 4.0 ) +
                       0.5 );  // qb: smaller particles.  was 320.0
    d_pix_shift = 8 - (int)( (float)r_refdef.vrect.width / 800.0 +
                             0.5 );  // qb: smaller particles.  was 320.0
    if ( d_pix_max < 1 )
        d_pix_max = 1;
...
void R_PrintDSpeeds( void ) {
    ...
    ri.Con_Printf( PRINT_ALL, "%3i %2ip %2iw %2ib %2is %2ie %2ia\n", ms,
                   dp_time, rw_time, db_time, se_time, de_time, da_time );
}
...
__declspec( naked ) void TransformVector( vec3_t vin, vec3_t vout ) {
    __asm mov eax, dword ptr[esp + 4] __asm mov edx,
        ...
        __asm fld dword ptr[eax + 0] __asm fmul dword ptr[vright + 0] __asm fld
            dword ptr[eax + 0] __asm fmul dword ptr[vup + 0] __asm fld dword
                ptr[eax + 0] __asm fmul dword ptr[vpn + 0]

        __asm fld dword ptr[eax + 4] __asm fmul dword ptr[vright + 4] __asm fld
            dword ptr[eax + 4] __asm fmul dword ptr[vup + 4] __asm fld dword
                ptr[eax + 4] __asm fmul dword ptr[vpn + 4]
        ...
        st( 0 )
            ...
            __asm fld dword ptr[eax + 8] __asm fmul dword
                ptr[vright + 8] __asm fld dword ptr[eax + 8] __asm fmul dword
                    ptr[vup + 8] __asm fld dword ptr[eax + 8] __asm fmul dword
                        ptr[vpn + 8]
        ...
        st( 0 )
            ...
            __asm fstp dword ptr[edx + 8] __asm fstp dword
                ptr[edx + 4] __asm fstp dword ptr[edx + 0]
...
void R_TransformPlane( mplane_t *p, float *normal, float *dist ) {
    ...
    *dist = p->dist - d;
    // TODO: when we have rotating entities, this will need to use the view
    // matrix
...
void R_SetupFrame( void ) {
    ...
    // if (r_dowarp)
    if ( r_dowarp && r_warpbuffer ) {  // warp into off screen buffer
        vrect.x = 0;
        vrect.y = 0;

        // vrect.width = r_newrefdef.width < WARP_WIDTH ? r_newrefdef.width :
        // WARP_WIDTH; vrect.width = r_newrefdef.width;
        vrect.width = r_warpwidth;
        // vrect.height = r_newrefdef.height < WARP_HEIGHT ? r_newrefdef.height
        // : WARP_HEIGHT; vrect.height = r_newrefdef.height;
