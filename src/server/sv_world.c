...
void SV_LinkEdict( edict_t *ent ) {
    ...
    if ( ent->solid == SOLID_BBOX &&
         !( ent->svflags &
            SVF_DEADMONSTER ) ) {  // assume that x/y are equal and symetric
        i = ent->maxs[0] / 8;
        if ( i < 1 )
            i = 1;
        if ( i > 31 )
    ...
    if ( num_leafs >= MAX_TOTAL_ENT_LEAFS ) {  // assume we missed some leafs,
                                               // and mark by headnode
        ent->num_clusters = -1;
        ...
        for ( i = 0; i < num_leafs; i++ ) {
            ...
            if ( j == i ) {
                if ( ent->num_clusters ==
                     MAX_ENT_CLUSTERS ) {  // assume we missed some leafs, and
                                           // mark by headnode
                    ent->num_clusters = -1;
                    ent->headnode = topnode;
                    break;
