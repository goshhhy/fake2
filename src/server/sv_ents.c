...
void SV_EmitPacketEntities( client_frame_t *from, client_frame_t *to,
    ...
    while ( newindex < to->num_entities || oldindex < from_num_entities ) {
        ...
        if ( newnum <
             oldnum ) {  // this is a new entity, send it from the baseline
        ...
        if ( newnum >
             oldnum ) {  // the old entity isn't present in the new message
            bits = U_REMOVE;
            if ( oldnum >= 256 )
...
void SV_WriteFrameToClient( client_t *client, sizebuf_t *msg ) {
    ...
    if ( client->lastframe <= 0 ) {  // client is asking for a retransmit
        ...
        lastframe = -1;
                ...
                ( UPDATE_BACKUP - 3 ) ) {  // client hasn't gotten a good
                                           // message through in a long time
...
void SV_BuildClientFrame( client_t *client ) {
    ...
    for ( e = 1; e < ge->num_edicts; e++ ) {
        ...
        if ( ent != clent ) {
            // check area
            if ( !CM_AreasConnected(
                     clientarea, ent->areanum ) ) {  // doors can legally
                                                     // straddle two areas, so
            ...
            if ( ent->s.renderfx & RF_BEAM ) {
                ...
                if ( ent->num_clusters ==
                     -1 ) {  // too many leafs for individual check, go by
                             // headnode
                ...
                if ( !ent->s.modelindex ) {  // don't send sounds if they will
                                             // be attenuated away
                    vec3_t delta;
                    float len;
