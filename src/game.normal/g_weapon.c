...
static void check_dodge( edict_t *self, vec3_t start, vec3_t dir, int speed ) {
    ...
    if ( ( tr.ent ) && ( tr.ent->svflags & SVF_MONSTER ) &&
         ( tr.ent->health > 0 ) && ( tr.ent->monsterinfo.dodge ) &&
         infront( tr.ent, self ) ) {
...
static void Grenade_Explode( edict_t *ent ) {
    ...
    // FIXME: if we are onground then raise our Z just a bit since we are a
    // point?
    if ( ent->enemy ) {
        float points;
        vec3_t v;
        ...
        if ( ent->spawnflags & 1 ) {
            if ( random() > 0.5 )
                gi.sound( ent, CHAN_VOICE,
                          gi.soundindex( "weapons/hgrenb1a.wav" ), 1, ATTN_NORM,
                          0 );
            else
                gi.sound( ent, CHAN_VOICE,
                          gi.soundindex( "weapons/hgrenb2a.wav" ), 1, ATTN_NORM,
                          0 );
        } else {
...
void rocket_touch( edict_t *ent, edict_t *other, cplane_t *plane,
    ...
    if ( other->takedamage ) {
        ...
        if ( !deathmatch->value && !coop->value ) {
            if ( ( surf ) &&
                 !( surf->flags & ( SURF_WARP | SURF_TRANS33 | SURF_TRANS66 |
                                    SURF_FLOWING ) ) ) {
                n = rand() % 5;
                while ( n-- )
