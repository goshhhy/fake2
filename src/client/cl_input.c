...
extern unsigned long long int sys_frame_time;
unsigned long long int frame_msec;
unsigned long long int old_sys_frame_time;
...
void KeyUp( kbutton_t *b ) {
    ...
    if ( c[0] )
        k = atoi( c );
    else {  // typed manually at the console, assume for unsticking, so clear
            // all
        b->down[0] = b->down[1] = 0;
    ...
    *cmd = CL_CreateCmd();
    ...
    if ( cmd->buttons && cl.cinematictime > 0 && !cl.attractloop &&
         cls.realtime - cl.cinematictime >
             1000 ) {  // skip the rest of the cinematic
        SCR_FinishCinematic();
    }
