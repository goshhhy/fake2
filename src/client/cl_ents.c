...
void CL_DeltaEntity( frame_t *frame, int newnum, entity_state_t *old,
    ...
    if ( ent->serverframe !=
         cl.frame.serverframe -
             1 ) {  // wasn't in last update, so initialize some things
...
void CL_ParsePacketEntities( frame_t *oldframe, frame_t *newframe ) {
    ...
    while ( 1 ) {
        ...
        while ( oldnum < newnum ) {  // one or more entities from the old packet
                                     // are unchanged
        ...
        if ( bits & U_REMOVE ) {  // the entity present in oldframe is not in
                                  // the current frame
            if ( cl_shownet->value == 3 )
                Com_Printf( "   remove: %i\n", newnum );
            if ( oldnum != newnum )
    ...
    while ( oldnum !=
            99999 ) {  // one or more entities from the old packet are unchanged
        if ( cl_shownet->value == 3 )
            Com_Printf( "   unchanged: %i\n", oldnum );
        CL_DeltaEntity( newframe, oldnum, oldstate, 0 );
...
void CL_ParseFrame( void ) {
    ...
    if ( cl.frame.deltaframe <= 0 ) {
        ...
        if ( old->serverframe !=
             cl.frame.deltaframe ) {  // The frame that the server did the delta
                                      // from
...
void CL_AddPacketEntities( frame_t *frame ) {
    ...
    for ( pnum = 0; pnum < frame->num_entities; pnum++ ) {
        ...
        if ( renderfx &
             ( RF_FRAMELERP |
               RF_BEAM ) ) {  // step origin discretely, because the frames
        ...
        if ( renderfx & RF_BEAM ) {  // the four beam colors are encoded in 32
                                     // bits of skinnum (hack)
            ent.alpha = 0.30;
        ...
        if ( effects & EF_COLOR_SHELL ) {
            ...
            // if we're in the rogue pack, set up the custom mixing, otherwise
            // just keep going
            //			if(Developer_searchpath(2) == 2)
            //			{
            // all of the solo colors are fine.  we need to catch any of the
            // combinations that look bad (double & half) and turn them into the
            // appropriate color, and make double/quad something special
            ...
            if ( renderfx & RF_SHELL_DOUBLE ) {
                if ( Developer_searchpath( 2 ) == 2 ) {
                    // lose the yellow shell if we have a red, blue, or green
                    // shell
                    if ( renderfx &
                    ...
                    if ( renderfx & RF_SHELL_RED )
                        renderfx |= RF_SHELL_BLUE;
                    // if we have a blue shell (and not a red shell), turn it to
                    // cyan by adding green
...
void CL_CalcViewValues( void ) {
    ...
    if ( ( cl_predict->value ) &&
         !( cl.frame.playerstate.pmove.pm_flags &
            PMF_NO_PREDICTION ) ) {  // use predicted values
        unsigned delta;
    ...
    if ( cl.frame.playerstate.pmove.pm_type <
         PM_DEAD ) {  // use predicted values
        for ( i = 0; i < 3; i++ )
