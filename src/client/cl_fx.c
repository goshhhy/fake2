...
===============
*/
void CL_NewDlight( int key, float x, float y, float z, float radius,
                   float time ) {
    cdlight_t *dl;
...
void CL_ParseMuzzleFlash( void ) {
    ...
    switch ( weapon ) {
        ...
        case MZ_HEATBEAM:
            ...
            dl->die = cl.time + 100;
            //		S_StartSound (NULL, i, CHAN_WEAPON,
            //S_RegisterSound("weapons/bfg__l1a.wav"), volume, ATTN_NORM, 0);
            break;
        case MZ_BLASTER2:
            dl->color[0] = 0;
            dl->color[1] = 1;
...
void CL_ParseMuzzleFlash2( void ) {
    ...
    switch ( flash_number ) {
        ...
        case MZ2_FLYER_BLASTER_2:
            ...
            dl->color[2] = 0;
            S_StartSound( NULL, ent, CHAN_WEAPON,
                          S_RegisterSound( "flyer/flyatck3.wav" ), 1, ATTN_NORM,
                          0 );
            break;
        ...
        case MZ2_MEDIC_BLASTER_1:
            ...
            dl->color[2] = 0;
            S_StartSound( NULL, ent, CHAN_WEAPON,
                          S_RegisterSound( "medic/medatck1.wav" ), 1, ATTN_NORM,
                          0 );
            break;
        ...
        case MZ2_HOVER_BLASTER_1:
            ...
            dl->color[2] = 0;
            S_StartSound( NULL, ent, CHAN_WEAPON,
                          S_RegisterSound( "hover/hovatck1.wav" ), 1, ATTN_NORM,
                          0 );
            break;
        ...
        case MZ2_TURRET_ROCKET:  // PGM
            ...
            dl->color[2] = 0.2;
            S_StartSound( NULL, ent, CHAN_WEAPON,
                          S_RegisterSound( "chick/chkatck2.wav" ), 1, ATTN_NORM,
                          0 );
            break;
        ...
        case MZ2_MAKRON_BLASTER_17:
            ...
            dl->color[2] = 0;
            S_StartSound( NULL, ent, CHAN_WEAPON,
                          S_RegisterSound( "makron/blaster.wav" ), 1, ATTN_NORM,
                          0 );
            break;
...
void CL_AddDLights( void ) {
    ...
    if ( vidref_val == VIDREF_GL ) {
        for ( i = 0; i < MAX_DLIGHTS; i++, dl++ ) {
            if ( !dl->radius )
                continue;
            V_AddLight( dl->origin, dl->radius, dl->color[0], dl->color[1],
                        dl->color[2] );
        }
    } else {
        for ( i = 0; i < MAX_DLIGHTS; i++, dl++ ) {
...
void CL_LogoutEffect( vec3_t org, int type ) {
    ...
    for ( i = 0; i < 500; i++ ) {
        ...
        for ( j = 0; j < 3; j++ ) p->vel[j] = crand() * 20;
        ...
        case EV_FOOTSTEP:
            if ( cl_footsteps->value )
                S_StartSound( NULL, ent->number, CHAN_BODY,
                              cl_sfx_footsteps[rand() & 3], 1, ATTN_NORM, 0 );
            break;
        case EV_FALLSHORT:
