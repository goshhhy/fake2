...
qboolean CL_CheckOrDownloadFile( char *filename ) {
    ...
    if ( FS_LoadFile( filename, NULL ) !=
         -1 ) {  // it exists, no need to download
        return true;
    }
...
void CL_ParseServerData( void ) {
    ...
    if ( cl.playernum ==
         -1 ) {  // playing a cinematic or showing a pic, not a level
...
void CL_LoadClientinfo( clientinfo_t *ci, char *s ) {
    ...
    if ( cl_noskins->value || *s == 0 ) {
        ...
        *t = 0;
        ...
        // if we still don't have a skin, it means that the male model didn't
        // have it, so default to grunt
        if ( !ci->skin ) {
            // see if the skin exists for the male model
            Com_sprintf( skin_filename, sizeof( skin_filename ),
    ...
    while ( 1 ) {
        ...
        switch ( cmd ) {
            ...
            case svc_reconnect:
                ...
                cls.connect_time =
                    -99999;  // CL_CheckForResend() will fire immediately
                break;
