...
void CL_CheckPredictionError( void ) {
    ...
    {                 // a teleport or something
        ...
        for ( i = 0; i < 3; i++ ) cl.prediction_error[i] = delta[i] * 0.125;
    }
}
...
void CL_ClipMoveToEntities( vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end,
    ...
    for ( i = 0; i < cl.frame.num_entities; i++ ) {
        ...
        trace = CM_TransformedBoxTrace( start, end, mins, maxs, headnode,
                                        MASK_PLAYERSOLID, ent->origin, angles );
...
void CL_PredictMovement( void ) {
    ...
    if ( !cl_predict->value || ( cl.frame.playerstate.pmove.pm_flags &
                                 PMF_NO_PREDICTION ) ) {  // just set angles
        for ( i = 0; i < 3; i++ ) {
