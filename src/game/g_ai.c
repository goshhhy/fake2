...
void ai_run( edict_t *self, float dist ) {
    ...
    if ( coop->value ) {  // FIXME: insane guys get mad with this, which causes
                          // crashes!
        if ( FindTarget( self ) )
            return;
    }
    ...
    if ( self->monsterinfo.aiflags & AI_PURSUE_NEXT ) {
        ...
        //dprint(ftos(vlen(self.origin - self.last_sighting))); dprint("\n");
    ...
    if ( new ) {
        ...
        if ( tr.fraction < 1 ) {
            ...
            if ( left >= center && left > right ) {
                ...
                //				debug_drawline(self.origin, self.last_sighting,
                //152);
